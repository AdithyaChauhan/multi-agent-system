...
def create_products(df):
    ...
    try:
        for idx, row in df.iterrows():
            ...
            # Parse attributes
            attributes = parse_attributes(row.get('Product Specification'), row.get('Technical Details'))
        ...
        for product in products:
            if product.attributes:
                ...
                for key, value in product.attributes.items():
                    spec = Spec(product_id=product.product_id, spec_key=key, spec_value=str(value))
                    specs.append(spec)
...
def create_demo_users_and_orders(products):
    ...
    try:
        ...
        users = [
            User(
                user_id="demo-user-1", email="alice@demo.com", name="Alice Demo", google_id=None, auth_provider="demo"
            ),
            User(user_id="demo-user-2", email="bob@demo.com", name="Bob Demo", google_id=None, auth_provider="demo"),
            User(
                user_id="demo-user-3",
